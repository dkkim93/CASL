...
extern "C" {
  ...
  void reset_game(ALEInterface *ale) {
      ale->environment->reset_sound();// clear buffer
	  ale->reset_game();
  }
  ...
  // dkk
  void get_rgb_audio(ALEInterface *ale, unsigned char *image_buffer, unsigned char *audio_buffer) {
    // Audio
    ale->environment->copy_sound();
    std::vector<uInt8> sound_data = ale->environment->sound_data;

    if (sound_data.size() == 0) {
      uInt8 *q = audio_buffer;
      for (size_t i=0; i<512; i++) {
        *q = 0; q++;
      }
    }

    else if ((sound_data.size() > 0) && (sound_data.size() < 512)) {
      uInt8 *q = audio_buffer;
      for (size_t i=0; i<sound_data.size(); i++) {
        *q = sound_data[i]; q++;
      }
	  for (size_t i=sound_data.size(); i<512; i++){
		*q = 0; q++;
	  }
    }
      ...
      uInt8 *q = audio_buffer;
      for (size_t i=sound_data.size()-512; i<sound_data.size(); i++) {
        *q = sound_data[i]; q++;
      }
    }
    sound_data.clear();
    ...
    size_t screen_size = w*h;
    pixel_t *ale_screen_data = ale->getScreen().getArray();
    ale->theOSystem->colourPalette().applyPaletteRGB(image_buffer, ale_screen_data, screen_size);
  }
