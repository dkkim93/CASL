...
ale_lib.get_rgb_audio.argtypes = [c_void_p, c_void_p, c_void_p]# dkk
ale_lib.get_rgb_audio.restype = None# dkk
...
class ALEInterface(object):
    ...
    def get_rgb_audio(self, screen_data=None, audio_data=None):
        """ dkk
        """
        # None 
        if(screen_data is None):
        ...
        if(audio_data is None):
          audio_data = np.empty(2000, dtype=np.uint8)

        # Data
        ale_lib.get_rgb_audio(self.obj, as_ctypes(screen_data[:]), as_ctypes(audio_data[:]))
