    ...
    myNumRecordSamplesNeeded(0)
{
    ...
    if (osystem->settings().getBool("sound") == 1) {
        // std::string filename = osystem->settings().getString("record_sound_filename");
        std::string filename = "dummy.wav";// dkk: dummy filename in which will not be used
...
// dkk
std::vector<uInt8> SoundSDL::copy_sound_data() {
  return mySoundExporter->m_data;
}

void SoundSDL::clear_sound() {
  mySoundExporter->clear_sound_data();
}
