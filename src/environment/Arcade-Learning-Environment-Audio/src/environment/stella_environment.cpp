...
reward_t StellaEnvironment::act(Action player_a_action, Action player_b_action) {
  ...
  for (size_t i = 0; i < m_frame_skip; i++) {
    ...
    // is not enabled)
    // If ... dkk
    if (i == 0) { m_osystem->sound().recordNextFrame(); }
...
// dkk
void StellaEnvironment::copy_sound() {
    sound_data = m_osystem->sound().copy_sound_data();
}

void StellaEnvironment::reset_sound() {
  m_osystem->sound().clear_sound();
}
