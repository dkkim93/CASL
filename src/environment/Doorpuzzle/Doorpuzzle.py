...
class Doorpuzzle():
    ...
    def _take_action(self, action):
        ...
        if hasattr(self.actions, 'UP') and action == self.actions.UP:
            ...
            new_loc = self.agent_loc[0] + 1
            if new_loc >= self.env_row:
                new_loc = self.env_row - 1  # Out of Boundary
            ...
            if new_loc >= self.env_col:
                new_loc = self.env_col - 1  # Out of Boundary
    ...
    def step(self, action, pid=None, count=None):
        ...
        # Reward
        if np.array_equal(self.agent_loc, self.key_loc) and self.has_key is False:
