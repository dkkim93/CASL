...
class Minecraft():
    ...
    def reset(self, reset_time = True):
        ...
        return self._get_obs()# Send back initial obs
    ...
    def _take_action(self, action):
        if hasattr(self.actions,'NULL') and action == self.actions.NULL:
            dummy = 1 # No action is taken for NULL action
    ...
    def dist_euclid(self, loc_a, loc_b):
        ...
        if dist > 1000:
            raise RuntimeError("[ ERROR ] Dist is too large! Something is wrong.")
