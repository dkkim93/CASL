...
def lstm_layer(input, input_dim, out_dim, initial_state_input, seq_lengths, name, reuse=False):
    ...
    with tf.variable_scope(name, reuse=reuse):
        if Config.USE_CUDNN:
            cell = CudnnLSTM(num_layers = 1, num_units = out_dim, input_size = input_dim)  
            #  params_size_t  = cell.params_size()
            params_size_t = ((input_dim * out_dim * 4) + (out_dim * out_dim * 4) + (out_dim * 2 * 4)) * 1 # See https://github.com/tensorflow/tensorflow/issues/6620#issuecomment-297159897
            batch_size = 9#tf.shape(seq_lengths)[0] # TODO totally wrong, remove later, just for testing
            input_reshaped = tf.reshape(input, [batch_size, -1, input_dim]) # TODO totally wrong, check LSTM case below for correct thing. just did this to help with swapping of time major
            input_reshaped = transpose_batch_time(input_reshaped) # LSTMBlockFusedCell expects time_major = True
            input_h = tf.Variable(tf.zeros([1,batch_size,out_dim])) # TODO totally wrong cudnn should use seqlength somehow
            input_c = tf.Variable(tf.zeros([1,batch_size,out_dim]))
            params = tf.Variable(tf.ones([params_size_t]), validate_shape = True)
            outputs, state_h, state_c = cell(input_data = input_reshaped,
                                             input_h = input_h,
                                             input_c = input_c,
                                             params = params,
                                             is_training = True)
            ...
            return outputs, [[state_h], [state_c]], batch_size
        else:
